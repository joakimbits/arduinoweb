/* Real-time tranciever web client for a remote baseband using the Arduino 
sensor platform. Similar to http://www.4now.net/arduino but without baseband. 

Wiring on Arduino Uno:
PIN    TYPE          FUNCTION
Pin 2  Logic output  Signal output.
Pin 3  Logic input   Signal detected.
Pin A0 Analog input  Signal strength.
Pin 13 Logic output  Stream active.

Other resources used:
ADC     Measure signal strength.
Timer2  Clock and alarms.

RECEIVER
Stores clock-intervals for signal-change events into a 256-cyclic buffer 
'cycles[edge]' and adc-values for the signal strengths measured after those 
events into 'magnitudes[edge]'. The last bit of the index indicates the 
direction of the edge. If the signal changed significantly, the time and 
value is reported.

TRANSMITTER
Reads time-intervals for signal transmissions from a 256-cyclic buffer 
'ticks[edge0]'. The last bit of the index indicates wether a signal should be 
output or not. Transmission is stopped when edge0 reaches edge1.

PROTOCOL
Oregon Scientific RF protocol version 2.1. This protocol uses Manchester-coding
with two repeated RF symbols per bit at 1200 Baud. Each bit is therefore 
4/1200Hz = 10/3 us long, with 1010 representing a bit change and 1100 a bit that
did not change value.

APPLICATION
Show measured signal strentghs values as fake temperature readings on a
weather station capable of receiving OS 2.1 packets.

Plan: Follow recipy http://community.pachube.com/node/112 for an umodified
ethernet shield connected to a sensor server.
*/
